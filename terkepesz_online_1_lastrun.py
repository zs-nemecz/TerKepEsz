...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\Zsuzsa\\HCCCL\\experiment\\computer_based_tasks\\terkepesz\\MR_version\\TerKepEsz\\terkepesz_online_1_lastrun.py',
...
# Initialize components for Routine "inter_session_break"
inter_session_breakClock = core.Clock()
w_size = win.size
...
for thisRec_practice_block in rec_practice_blocks:
    ...
    for thisRec_practice_trial in rec_practice_trials:
        ...
        # ------Prepare to start Routine "inter_session_break"-------
        continueRoutine = True
        # update component parameters for each repeat
        w_size = win.size
        ...
        # keep track of which components have finished
        inter_session_breakComponents = [rec_fx_interior, rec_fx_cross, rec_fx_key, rec_fx_text_block, rec_fx_instructions_text]
        for thisComponent in inter_session_breakComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        t = 0
        _timeToFirstFrame = win.getFutureFlipTime(clock="now")
        inter_session_breakClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
        frameN = -1
        
        # -------Run Routine "inter_session_break"-------
        while continueRoutine:
            # get current time
            t = inter_session_breakClock.getTime()
            tThisFlip = win.getFutureFlipTime(clock=inter_session_breakClock)
            tThisFlipGlobal = win.getFutureFlipTime(clock=None)
            frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
            # update/draw components on each frame
        ...
        # -------Ending Routine "inter_session_break"-------
        for thisComponent in inter_session_breakComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        ...
        # the Routine "inter_session_break" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
