...
flowScheduler.add(end_of_encodingRoutineBegin());
flowScheduler.add(end_of_encodingRoutineEachFrame());
flowScheduler.add(end_of_encodingRoutineEnd());
...
var end_of_encodingClock;
var end_of_encoding_text;
var end_of_encoding_key;
var globalClock;
var routineTimer;
function experimentInit() {
  ...
  end_of_encodingClock = new util.Clock();
  end_of_encoding_text = new visual.TextStim({
    win: psychoJS.window,
    name: 'end_of_encoding_text',
  ...
  end_of_encoding_key = new core.Keyboard({psychoJS: psychoJS, clock: new util.Clock(), waitForStart: true});
...
function enc_practice_fxRoutineBegin(trials) {
  return function () {
    ...
    enc_practice_fxComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
...
function enc_practice_trialRoutineBegin(trials) {
  return function () {
    ...
    enc_practice_trialComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
...
function start_MRRoutineBegin(trials) {
  return function () {
    ...
    start_MRComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
...
var _end_of_encoding_key_allKeys;
var end_of_encodingComponents;
function end_of_encodingRoutineBegin(trials) {
  return function () {
    //------Prepare to start Routine 'end_of_encoding'-------
    t = 0;
    end_of_encodingClock.reset(); // clock
    frameN = -1;
    routineTimer.add(120.000000);
    // update component parameters for each repeat
    end_of_encoding_key.keys = undefined;
    end_of_encoding_key.rt = undefined;
    _end_of_encoding_key_allKeys = [];
    // keep track of which components have finished
    end_of_encodingComponents = [];
    end_of_encodingComponents.push(end_of_encoding_text);
    end_of_encodingComponents.push(end_of_encoding_key);
    
    end_of_encodingComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
...
function end_of_encodingRoutineEachFrame(trials) {
  return function () {
    //------Loop for each frame of Routine 'end_of_encoding'-------
    let continueRoutine = true; // until we're told otherwise
    // get current time
    ...
    if (t >= 0.0 && end_of_encoding_text.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      end_of_encoding_text.tStart = t;  // (not accounting for frame time here)
    ...
    if (end_of_encoding_text.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      end_of_encoding_text.setAutoDraw(false);
    }
    
    // *end_of_encoding_key* updates
    if (t >= 0.0 && end_of_encoding_key.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      end_of_encoding_key.tStart = t;  // (not accounting for frame time here)
      end_of_encoding_key.frameNStart = frameN;  // exact frame index
      
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function() { end_of_encoding_key.clock.reset(); });  // t=0 on next screen flip
      psychoJS.window.callOnFlip(function() { end_of_encoding_key.start(); }); // start on screen flip
      psychoJS.window.callOnFlip(function() { end_of_encoding_key.clearEvents(); });
    }

    frameRemains = 0.0 + 120 - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (end_of_encoding_key.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      end_of_encoding_key.status = PsychoJS.Status.FINISHED;
  }

    if (end_of_encoding_key.status === PsychoJS.Status.STARTED) {
      let theseKeys = end_of_encoding_key.getKeys({keyList: ['right', 'space'], waitRelease: false});
      _end_of_encoding_key_allKeys = _end_of_encoding_key_allKeys.concat(theseKeys);
      if (_end_of_encoding_key_allKeys.length > 0) {
        end_of_encoding_key.keys = _end_of_encoding_key_allKeys[_end_of_encoding_key_allKeys.length - 1].name;  // just the last key pressed
        end_of_encoding_key.rt = _end_of_encoding_key_allKeys[_end_of_encoding_key_allKeys.length - 1].rt;
        // a response ends the routine
        continueRoutine = false;
      }
    }
...
function end_of_encodingRoutineEnd(trials) {
  return function () {
    ...
    psychoJS.experiment.addData('end_of_encoding_key.keys', end_of_encoding_key.keys);
    if (typeof end_of_encoding_key.keys !== 'undefined') {  // we had a response
        psychoJS.experiment.addData('end_of_encoding_key.rt', end_of_encoding_key.rt);
        routineTimer.reset();
        }
    
    end_of_encoding_key.stop();
    return Scheduler.Event.NEXT;
  };
}
