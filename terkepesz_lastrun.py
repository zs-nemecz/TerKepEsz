...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\Zsuzsa\\HCCCL\\experiment\\computer_based_tasks\\terkepesz\\terkepesz_encoding\\terkepesz_lastrun.py',
...
for thisEnc_practice_trial in enc_practice_trials:
    ...
    for thisComponent in enc_practice_trialComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # the Routine "enc_practice_trial" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    for thisComponent in enc_practice_feedbackComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # the Routine "enc_practice_feedback" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
...
for thisEnc_run in enc_runs:
    ...
    enc_trials = data.TrialHandler(nReps=1, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions(encoding_table, selection=encoding_selection),
    ...
    for thisComponent in start_rec_blockComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    enc_runs.addData('start_rec_block_text.started', start_rec_block_text.tStartRefresh)
    enc_runs.addData('start_rec_block_text.stopped', start_rec_block_text.tStopRefresh)
    
    # set up handler to look after randomisation of conditions etc
    rec_trials = data.TrialHandler(nReps=1, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions(recognition_table, selection=recognition_selection),
